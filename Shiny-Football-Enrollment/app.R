...
server <- function(input, output) {
    ...
    output$summary <- renderUI({
      HTML(paste(
        ...
        br(),
        h4("Other Factors Could Explain Results"),
        div("Among the colleges with the highest r-squared values, many rank towards the top of their conferences but are not perceived as having the very best football programs within their conferences. Auburn, Oklahoma State, and Washington State come to mind. Colleges that are football powerhouses, such as Alabama and Oregon, have some of the lowest observed r-squared values. Perhaps applicants assume that top football schools will routinely perform well in their conferences, regardless of the results of any one season, and this is why correlations for these colleges are weak. Meanwhile, applicants become excited when colleges that are second or third in their conferences have good seasons (perhaps thinking that next season will be the season when these schools will finally win their conference or make it to the national championship game), leading to a noticeable increase in applications."),
        br(),
        div(
